...
describe("the slash-mention prompt path", () => {
  ...
  it("sends a command mention (bb's builtin review) as the literal text", async () => {
    // bb builtins travel as literal text — EXCEPT /compact, which the bridge
    // maps onto prime's compaction (bbpa-ggf.6); review is a plain builtin.
    const providerThreadId = await startThread("s", "thr_builtin");
    daemon.enqueuePrompt({ events: textTurnEvents({ text: "ok" }) });
    sendRequest("t", "turn/start", {
    ...
    expect(promptMessages()[0]!.message).toBe("/review");
  });
