...
**Turn reconciliation**:
Доведение зависшего открытого хода bb до конца силами bridge: пока ход открыт, lane опрашивает `get_state` resident-сессии; если сессия подтверждённо не работает (не стримит, нет запущенных инструментов/bash/RLM-детей, не идёт компакция) дольше grace-периода, bridge сам закрывает ход как `interrupted` (реальный исход прогона неизвестен). Живой worker закрывает с заметкой в ленте; проверка после adopt-attach — тихим boundary `claimIfIdle`. Мотивация: свой watchdog ходов bb убрал (wire fact), конец хода теряется вместе с переиспользованным bridge-воркером или пропавшим `agent_end`.
_Avoid_: watchdog (это убранный механизм bb), «kill зависшего хода» (ничего не убивается — prime-side прогон уже завершился)
