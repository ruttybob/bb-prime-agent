...
**Child thread**:
Тред bb, который плагин создал для RLM-ребёнка resident-сессии (bbpa-b1m.11, ADR-0005): `parentThreadId` указывает на тред родителя, привязка тред↔сессия ребёнка идёт через `thread/identity`, первый диспатч — поглощаемый мостом маркер с id сессии. Ребёнка можно читать и писать в него; stop треда только отвязывает.
_Avoid_: «форк треда» (fork копирует историю), сабтаск-строка (это ряд делегации в ленте родителя)
